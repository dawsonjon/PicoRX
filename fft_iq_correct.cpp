...
#define ALPHA ((int32_t)(32767 * 0.996f)) // time constant ~250 samples
#define ONE_MINUS_ALPHA ((int32_t)32767 - ALPHA)
...
void fft_iq_correct(int16_t *i, int16_t *q, uint16_t start_bin, uint16_t stop_bin)
{
    for (size_t k = start_bin; k <= stop_bin; k++)
    {
        ...
        // 2. Form complex conjugates
        cplx_q15_t Y_mir_conj = {.re = Y_mirror.re, .im = (-Y_mirror.im)};
        cplx_q15_t Y_k_conj = {.re = Y_k.re, .im = (-Y_k.im)};

        // 3. Fetch the correction coefficient W(k)
        const cplx_q15_t W = W_table[k];

        // 4. SYMMETRICAL CORRECTION PHASE
        // Correct Bin K:  Y_corr(k) = Y(k) - [ W(k) * Y*(-k) ]
        const cplx_q15_t W_times_Y_mir_conj = cplx_mul_q15(W, Y_mir_conj);
        ...
        const cplx_q15_t W_conj = {.re = W.re, .im = (-W.im)};
        const cplx_q15_t W_conj_times_Y_k_conj = cplx_mul_q15(W_conj, Y_k_conj);
        ...
        const cplx_q15_t error_term = cplx_mul_q15(Y_k_corrected, Y_mir_conj);

        int32_t delta_w_re = (ONE_MINUS_ALPHA * error_term.re) >> 15;
        int32_t delta_w_im = (ONE_MINUS_ALPHA * error_term.im) >> 15;

        // Update tracking table
        W_table[k].re = ((ALPHA * W_table[k].re) >> 15) + delta_w_re;
        W_table[k].im = ((ALPHA * W_table[k].im) >> 15) + delta_w_im;
    }
}
