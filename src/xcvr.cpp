...
void xcvr::tune_tx(bool transmit_enable)
{
  ...
  if (transmit_enable) {
    if (external_nco_active) {
      ...
      double adjusted_tuned_frequency_Hz = tuned_frequency_Hz * 1e6 / (1e6 + ppm);
      nco_frequency_Hz =
          nco_set_frequency(pio, sm, adjusted_tuned_frequency_Hz, system_clock_rate, 0, if_mode);
      offset_frequency_Hz = adjusted_tuned_frequency_Hz - nco_frequency_Hz;
    ...
    if (external_nco_active) {
      ...
      sem_acquire_blocking(&i2c_semaphore);
      nco_frequency_Hz = external_nco.set_frequency_hz(adjusted_tuned_frequency_Hz +
                                                       ((uint16_t)if_frequency_hz_over_100 * 100));
      sem_release(&i2c_semaphore);
      offset_frequency_Hz = adjusted_tuned_frequency_Hz - nco_frequency_Hz;
...
void xcvr::tune_rx()
{
  if (enable_external_nco) {
    ...
    if (internal_nco_active) {
      ...
      uint32_t vco_freq =
          (12000000 / possible_frequencies[0].refdiv) * possible_frequencies[0].fbdiv;
      set_sys_clock_pll(vco_freq, possible_frequencies[0].postdiv1,
                        possible_frequencies[0].postdiv2);
    ...
    if (!external_nco_initialised) {
      external_nco_good =
          external_nco.initialise(i2c1, PIN_DISPLAY_SDA, PIN_DISPLAY_SCL, 0x60, 25000000);
...
void xcvr::apply_settings()
{
  ...
  if (sem_try_acquire(&settings_semaphore)) {
    ...
    rx_dsp_inst.set_noise_reduction(settings_to_apply.enable_noise_reduction,
                                    settings_to_apply.noise_estimation,
                                    settings_to_apply.noise_threshold);
    ...
    for (int band = 0; band < NUM_BANDS; ++band) {
      if (settings_to_apply.tuned_frequency_Hz >
              (settings_to_apply.tx_band_limits.lower[band] * 50000) &&
          settings_to_apply.tuned_frequency_Hz <
              (settings_to_apply.tx_band_limits.upper[band] * 50000)) {
        tx_enable = true;
      }
    }
    ...
    if ((tuned_frequency_Hz != settings_to_apply.tuned_frequency_Hz) ||
        (ppm != settings_to_apply.ppm) || (if_mode != settings_to_apply.if_mode) ||
...
#ifdef WITH_TX
xcvr::xcvr(xcvr_settings& _settings_to_apply, xcvr_status& _status)
    : dit(PIN_DIT), dah(PIN_DAH), settings_to_apply(_settings_to_apply), status(_status)
#else
xcvr::xcvr(xcvr_settings& _settings_to_apply, xcvr_status& _status)
    : settings_to_apply(_settings_to_apply), status(_status)
#endif
{
  ...
  channel_config_set_dreq(&ping_cfg,
                          DREQ_ADC); // Pace transfers based on availability of ADC samples
  ...
  channel_config_set_dreq(&pong_cfg,
                          DREQ_ADC); // Pace transfers based on availability of ADC samples
...
void __not_in_flash_func(xcvr::process_block)(uint16_t adc_samples[], int16_t audio[])
{
  ...
  uint16_t num_samples =
      rx_dsp_inst.process_block(adc_samples, audio, stream_raw_iq ? &usb_ring_buffer : NULL);
  ...
  if (!stream_raw_iq) {
    ...
    ring_buffer_push_ovr(&usb_ring_buffer, (uint8_t*)tmp_audio, sizeof(int16_t) * 2 * num_samples);
  }
}
...
int32_t __not_in_flash_func(xcvr::get_tx_sample)(adc& mic_adc, cw_keyer& keyer)
{
  ...
  if (test_tone_setting == 1) {
    ...
    monitor = audio =
        sin_table[m_test_tone1_phase >> 21] / 2 + sin_table[m_test_tone2_phase >> 21] / 2;
...
void __not_in_flash_func(xcvr::transmit_polar_external)()
{
  ...
  while (!tx_complete) {
    ...
    for (uint16_t idx = 0; idx < 1000; idx++) {
      ...
      // demodulate
      s_debug debug;
      audio_modulator.process_sample(transmit_mode, audio, i, q, next_magnitude, next_phase,
                                     fm_deviation_f15, debug);
      ...
      if (toggle) { // only output at half rate when using external nco
        ...
        if (magnitude >= MAG_THR) { // good phase
          ...
          if (next_magnitude >= MAG_THR) { // next phase good
            ...
            uint16_t positive_phase_change_f16 =
                phase_change_f16; // force a positive number half way to next phase
...
void __not_in_flash_func(xcvr::transmit_polar)()
{
  ...
  // create CW keyer
  cw_keyer keyer(tx_cw_paddle, tx_cw_speed,
                 rf_nco.get_sample_frequency_Hz(clock_frequency_Hz, waveforms_per_sample), dit,
                 dah);
  ...
  while (!tx_complete) {
    ...
    for (uint16_t idx = 0; idx < 1000; idx++) {
      ...
      // output magnitude
      magnitude_pwm.output_sample((magnitude + prev_magnitude) / 2, tx_pwm_min, tx_pwm_max,
                                  tx_pwm_threshold);
...
void xcvr::run()
{
  ...
  while (true) {
    ...
    dma_channel_configure(adc_dma_ping, &ping_cfg, ping_samples, &adc_hw->fifo, adc_block_size,
                          false);
    dma_channel_configure(adc_dma_pong, &pong_cfg, pong_samples, &adc_hw->fifo, adc_block_size,
                          false);
