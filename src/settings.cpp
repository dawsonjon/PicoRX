...
void apply_settings_to_xcvr(xcvr& transceiver, xcvr_settings& xcvr_settings, s_settings& settings,
                            bool suspend, bool settings_changed)
{
  ...
  xcvr_settings.tx_treble = settings.global.tx_treble;
  xcvr_settings.tx_bass = settings.global.tx_bass;
  xcvr_settings.tx_compression = settings.global.tx_compression;
  ...
  xcvr_settings.tx_noise_gate = settings.global.tx_noise_gate;
...
void memory_store_channel(s_memory_channel memory_channel, uint16_t channel_number,
                          s_settings& settings, xcvr& transceiver, xcvr_settings& xcvr_settings)
{
  static_assert(sizeof(s_memory_channel) < memory_chan_size * 4);
  ...
  const uint32_t first_channel_in_sector =
      num_channels_per_sector * (channel_number / num_channels_per_sector);
  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  apply_settings_to_xcvr(transceiver, xcvr_settings, settings, true,
                         false);                       // suspend xcvr to disable all DMA transfers
  ...
  apply_settings_to_xcvr(transceiver, xcvr_settings, settings, false,
                         false); // resume xcvr operation
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void autosave_store_settings(s_settings settings, xcvr& transceiver, xcvr_settings& xcvr_settings)
{
  ...
  const uint32_t first_channel_in_sector =
      num_channels_per_sector * (empty_channel / num_channels_per_sector);
  ...
  for (uint16_t channel = 0; channel < num_channels_per_sector; channel++) {
    for (uint16_t location = 0; location < autosave_chan_size; location++) {
      if (channel + first_channel_in_sector < num_chans) {
        sector_copy[channel][location] =
            autosave_memory[channel + first_channel_in_sector][location];
      }
    }
  }
