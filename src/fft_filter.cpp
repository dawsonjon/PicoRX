...
void fft_filter::filter_block(int16_t sample_real[], int16_t sample_imag[], s_filter_control &filter_control, int16_t capture[]) {
  ...
  {
    for (uint16_t i = 0; i < fft_size; i++) {
      capture[i] = (((int32_t)capture[i]<<filter_control.spectrum_smoothing) - capture[i] + rectangular_2_magnitude(sample_real[i], sample_imag[i])) >> (filter_control.spectrum_smoothing-1);
    }
  }
