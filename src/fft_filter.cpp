...
#ifndef SIMULATION
void __not_in_flash_func(fft_filter::filter_block)(int16_t sample_real[], int16_t sample_imag[],
                                                   s_filter_control& filter_control,
                                                   int16_t capture[])
{
#else
void fft_filter::filter_block(int16_t sample_real[], int16_t sample_imag[],
                              s_filter_control& filter_control, int16_t capture[])
{
#endif
  ...
  if (filter_control.capture) {
    for (uint16_t i = 0; i < fft_size; i++) {
      capture[i] = (((int32_t)capture[i] << filter_control.spectrum_smoothing) - capture[i] +
                    rectangular_2_magnitude(sample_real[i], sample_imag[i])) >>
                   (filter_control.spectrum_smoothing);
    }
  }
  ...
  for (uint16_t i = 0; i < (new_fft_size / 2u) + 1; i++) {
    ...
    if (!filter_control.upper_sideband || i < filter_control.start_bin ||
        i > filter_control.stop_bin) {
      sample_real[i] = 0;
      sample_imag[i] = 0;
    } else {
  ...
  if (filter_control.enable_noise_reduction && filter_control.upper_sideband) {
    ...
    noise_reduction(sample_real, sample_imag, magnitudes, positive_noise_estimate,
                    positive_signal_estimate, start_bin, filter_control.stop_bin,
                    filter_control.noise_smoothing, filter_control.noise_threshold);
  }
  ...
  for (uint16_t i = 0; i < (new_fft_size / 2u) - 1; i++) {
    ...
    if (!filter_control.lower_sideband || bin < filter_control.start_bin ||
        bin > filter_control.stop_bin) {
      sample_real[new_idx] = 0;
      sample_imag[new_idx] = 0;
    } else {
      sample_real[new_idx] = cic_correct(bin, filter_control.fft_bin,
                                         sample_real[fft_size - (new_fft_size / 2u) + i + 1]);
      sample_imag[new_idx] = cic_correct(bin, filter_control.fft_bin,
                                         sample_imag[fft_size - (new_fft_size / 2u) + i + 1]);
  ...
  if (filter_control.enable_noise_reduction && filter_control.lower_sideband) {
    ...
    noise_reduction(&sample_real[new_fft_size / 2u], &sample_imag[new_fft_size / 2u], magnitudes,
                    negative_noise_estimate, negative_signal_estimate,
                    new_fft_size / 2u - 1 - filter_control.stop_bin,
                    new_fft_size / 2u - 1 - start_bin, filter_control.noise_smoothing,
  ...
  if (filter_control.enable_auto_notch) {
    ...
    if (peak_bin == last_peak_bin && confirm_count < confirm_threshold)
      confirm_count++;
    if (peak_bin != last_peak_bin && confirm_count > 0)
      confirm_count--;
    last_peak_bin = peak_bin;

    // remove highest bin
    if ((confirm_count > confirm_threshold / 2u) && (peak_bin > 3u) &&
        (peak_bin < new_fft_size - 3u)) {
...
#ifndef SIMULATION
void __not_in_flash_func(fft_filter::process_sample)(int16_t sample_iq[],
                                                     s_filter_control& filter_control,
                                                     int16_t capture[])
{
#else
void fft_filter::process_sample(int16_t sample_iq[], s_filter_control& filter_control,
                                int16_t capture[])
{
#endif
