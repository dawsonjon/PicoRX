                         ...
                         s_settings& settings)
{
  ...
  if (strncmp(cmd, "FA", 2) == 0) {
    ...
    if (cmd[2] == ';') {
      printf("IF%011lu     +0000000000%c%c00000 ;", settings.channel.frequency,
             mode_translation[settings.channel.mode], vfor);
    }
    ...
    if (cmd[134] == ';') {
      ...
      for (uint8_t word_idx = 0; word_idx < 16; ++word_idx) {
        char word_string[9];
        memcpy(word_string, &cmd[(word_idx * 8) + 6], 8);
        cmd[8] = 0;
      ...
      memory_store_channel(memory_channel, channel_number, settings, transceiver,
                           settings_to_apply);
