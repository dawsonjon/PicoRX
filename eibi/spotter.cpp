...
static float rad2deg(float r) { return r * 180.0 / M_PI; }
...
void c_spotter::lat_lon_to_pixel(uint16_t &x, uint16_t &y, float lon, float lat)
{
  ...
  if(lon_separation >= 360.0f) lon_separation -= 360.0f;
  if(lon_separation < 0.0f) lon_separation += 360.0f;
